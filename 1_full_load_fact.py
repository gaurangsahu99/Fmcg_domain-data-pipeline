...
# MAGIC %run /Workspace/Users/gaurangsahu1999@gmail.com/consolidated_pipeline/1_setup/utilities
...
base_path=f's3://sportsbar-1999/{data_source}'
...
# 2. clean customers_id->keep numeric else set to 999999
df_orders=df_orders.withColumn(
    "customer_id",
    F.when(F.col("customer_id").rlike("^[0-9]+$"), F.col("customer_id")).otherwise(999999)
    .cast("string")
    )
# 3. remove weekday from order_placement_date
df_orders=df_orders.withColumn(
...
# check what's the maximum and minimun date
df_orders.agg(
    F.min("order_placement_date").alias("min"),
    F.max("order_placement_date").alias("max")
).show()
...
# first change the date to first  day of the month 2025-08-08 --> 2025-08-01
df_monthly=(
    df_child
